...
impl Piece {
    ...
    pub fn generate_moves(&self, board : &Board, row: usize, col: usize) ->Option<Vec<Move>> {
        match self.piece_type {
            PieceType::Pawn => self.generate_pawn_moves(board, row, col),
    ...
    fn one_square_move(&self, board : &Board, initial_square : u8, row: usize, col: usize, new_row : usize) ->Option<Move> {
    ...
    fn two_square_move(&self, board : &Board, initial_square : u8, row: usize, col: usize, is_one_move_allowed :bool, direction : isize) ->Option<Move> {
       // Check if it's the pawn's first move and if the two-square move is available
       if (row == 1 && self.color == Color::White) || (row == 6 && self.color == Color::Black) {
    ...
    fn capture_moves(&self, board : &Board, initial_square : u8, row: usize, direction : isize, new_col : isize) ->Option<Move> {
    ...
    pub fn generate_pawn_moves(&self, board : &Board, row: usize, col: usize) ->Option<Vec<Move>> {
        ...
        // One square movement
        if let Some(move_) = self.one_square_move(board, initial_square, row, col, new_row) {
            is_one_move_allowed = true;
            moves.push(move_);
        }

        // Two square movement
        if let Some(move_) = self.two_square_move(board, initial_square, row, col, is_one_move_allowed, direction){
            moves.push(move_);
        }

        // Captures 
        if let Some(move_) = self.capture_moves(board, initial_square, row, direction, col as isize - 1){
            moves.push(move_);
        }
        if let Some(move_) = self.capture_moves(board, initial_square, row, direction, col as isize + 1){
            moves.push(move_);
...
mod tests{
    ...
    fn test_generate_pawn_moves_white() {
        ...
        let piece = Piece {piece_type:Pawn, color:White};
    ...
    fn test_generate_pawn_moves_black() {
        ...
        let col = 3;
        let square = row * BOARD_SIZE + col;
        let piece = Piece {piece_type:Pawn, color:Black};
        board.set_piece(square as u8, Some(piece));
        let moves = piece.generate_moves(&board, row, col).unwrap();
    ...
    fn test_generate_pawn_moves_white_only_one_square_move_allowed() {
        ...
        let white_pawn = Piece {piece_type:Pawn, color:White};
        let black_pawn = Piece {piece_type:Pawn, color:Black};
